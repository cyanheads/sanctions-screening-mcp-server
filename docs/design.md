...
| `sanctions_list_sources` | The watchlists and GLEIF datasets currently loaded, each with record count, source URL, and license, plus each mirror's readiness and as-of timestamp (one for the sanctions mirror, one for GLEIF). Provenance and freshness for any result. | *(none)* | `readOnlyHint`, `openWorldHint: false` |
...
| `SANCTIONS_REFRESH_CRON` | No (default `0 4 * * *`) | Cron for the scheduled refresh of the sanctions lists + name and identifier indexes (HTTP transport only). GLEIF deltas are applied manually with `mirror:refresh`. |
